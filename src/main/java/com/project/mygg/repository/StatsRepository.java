...
public interface StatsRepository extends JpaRepository<StatsEntity, Long> {
    ...
    @Query("SELECT new com.project.mygg.DTO.TotalKdaDTO(SUM(s.win),SUM(s.lose),SUM(s.kill),SUM(s.death),SUM(s.assist))" +
            "FROM StatsEntity s WHERE s.playerEntity.id= :playerId "
    )
    List<TotalKdaDTO> findTotalKdaByPlayer(@Param("playerId") Long playerId);
